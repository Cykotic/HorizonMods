...
async function slashLoader() {
    ...
    try {
        ...
        if (options.slash_global == true) {
            const global_data = await rest.put(Routes.applicationCommands(process.env.clientId), {
                body: slashCommands
            });
            console.log(chalk.white(`[${chalk.grey("SLASH")}]${chalk.white(" - ")}Successfully reloaded ${global_data.length} application commands`));
            console.log(" ");
        } else {
            const guild_data = await rest.put(Routes.applicationGuildCommands(process.env.clientId, process.env.guildId), {
                body: slashCommands
            });
